"""Batch SAM 3 combined tracking (playing cards + table devices) over many rounds.

Loads the SAM 3 video predictor ONCE and loops over N rounds. For each round it
runs two prompt passes ("playing card", "transparent plastic box") in one
session, assigns the 3 fixed devices by screen position, and saves ONLY the
combined H.264 mp4 + the small combined.json. Resumable (skips rounds whose
mp4 already exists) and fault-tolerant (one bad round won't kill the batch).

    python batch_combined_1000.py [N] [START]
